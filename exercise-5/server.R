...
#Load the dataset 'womensrole' from the HSAUR package (same way you would load a built in dataset)
womens.data <- HSAUR::womensrole
...
shinyServer(function(input, output) {
  ...
  output$plotNamehere <- renderPlot({
    ...
    if (input$sex == "Male") {
      plot.data <- womens.data %>% filter(sex == "Male")
    } else if (input$sex == "Female") {
      plot.data <- womens.data %>% filter(sex == "Female")
    } else {
      plot.data <- womens.data
    }
